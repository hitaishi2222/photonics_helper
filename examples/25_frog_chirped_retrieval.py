...
for chirp in [0.0, 2.0, 3.0]:
    ...
    result = retrieve(trace, max_iter=150, tol=1e-5, verbose=False,
                      seed=0, n_restarts=5)
    f = fidelity(trace, result)

    # Determinism: the same call with the same seed returns identical fields
    result2 = retrieve(trace, max_iter=150, tol=1e-5, verbose=False,
                       seed=0, n_restarts=5)
    ...
    print(f"  chirp C = {chirp:.1f} | fidelity {f:.4f} | seed-deterministic: "
          f"{identical}")
...
def align_env(true_env2, rec_env2):
    ...
    for s in range(len(A)):
        ...
        best = min(best, np.max(np.abs(A - roll)),
                   np.max(np.abs(A - roll[::-1])))
    return best
...
for chirp in [2.0, 3.0]:
    ...
    result = retrieve(trace, max_iter=150, tol=1e-5, verbose=False,
                      seed=0, n_restarts=5)
    env_err = align_env(np.abs(chirped_gaussian(chirp)) ** 2,
                        np.abs(result.field) ** 2)
    print(f"  chirp C = {chirp:.1f}: envelope error (best shift/reversal) "
...
for ax, chirp in zip(axes, [2.0, 3.0]):
    ...
    ax.set_title(f"chirp C = {chirp:.0f}, fidelity = "
                 f"{fidelity(trace, result):.4f}")
    ax.set_xlabel("t / T0")
    ax.set_ylabel("Re E(t)")
    ax.legend()
plt.tight_layout()
fig.savefig("examples/images/frog_chirped_retrieval.png", dpi=150,
            bbox_inches="tight")
