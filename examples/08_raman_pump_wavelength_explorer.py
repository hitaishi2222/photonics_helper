...
def main():
    ...
    print(
        f"Δν̃ (Raman):     {spec.raman_shift_cm:.0f} cm⁻¹ = {spec.raman_shift_THz:.2f} THz"
    )
    print()

    # ── Plot 1: Frequency axis (equidistant) ────────────────────────────────

    fig = explorer.plot_frequency_axis(
        pump, backend="matplotlib", freq_range_THz=30, figsize=(12, 4)
    )
    plt.savefig(
        "examples/images/08_raman_frequency_axis.png", dpi=150, bbox_inches="tight"
    )
    ...
    fig = explorer.plot_wavelength_axis(
        pump, backend="matplotlib", wl_range_nm=100, figsize=(12, 4)
    )
    plt.savefig(
        "examples/images/08_raman_wavelength_axis.png", dpi=150, bbox_inches="tight"
    )
    ...
    fig = explorer.plot_both(
        pump, backend="matplotlib", freq_range_THz=30, wl_range_nm=100, figsize=(16, 5)
    )
    plt.savefig("examples/images/08_raman_both_axes.png", dpi=150, bbox_inches="tight")
    ...
    fig, axes = plt.subplots(
        1, 4, figsize=(20, 5), gridspec_kw={"width_ratios": [1, 1, 1, 1]}
    )
    fig.suptitle(
        "Raman Wavelength Shift: Material Comparison (800 nm pump)",
        fontsize=14,
        fontweight="bold",
    )
    ...
    for idx, (name, mat_spec) in enumerate(materials.items()):
        ...
        ax.vlines(
            a, -0.1, 0.15, colors=colors[name], linewidth=3, label=f"AS ({a:.0f} nm)"
        )
        ax.vlines(
            wl_p,
            -0.1,
        ...
        ax.vlines(s, -0.1, 0.15, colors="#22c55e", linewidth=3, label=f"S ({s:.0f} nm)")

        delta_s = s - wl_p
        delta_a = wl_p - a
        ax.annotate(
            f"Δλ_S={delta_s:.1f} nm",
            xy=((wl_p + s) / 2, -0.05),
            ha="center",
            fontsize=8,
            color="#22c55e",
        ...
        ax.set_title(f"{name}  (Δν̃ = {mat_spec.raman_shift_cm:.0f} cm⁻¹)", fontsize=11)
        ax.set_yticks([])
    ...
    plt.tight_layout()
    plt.savefig(
        "examples/images/08_raman_material_comparison.png", dpi=150, bbox_inches="tight"
    )
    ...
    fig.suptitle(
        "Pump Wavelength Sweep — Stokes/Anti-Stokes vs Pump",
        fontsize=14,
        fontweight="bold",
    )
    ...
    for name, mat_spec in materials.items():
        exp = PumpWavelengthExplorer(spec=mat_spec)
        stokes_wls = [
            exp.pump_to_stokes(Wavelength(wl, "um")).as_um for wl in pump_wls_um
        ]
        ax1.plot(pump_wls_um, stokes_wls, color=colors[name], linewidth=2, label=name)
    ...
    for name, mat_spec in materials.items():
        exp = PumpWavelengthExplorer(spec=mat_spec)
        anti_wls = [
            exp.pump_to_anti_stokes(Wavelength(wl, "um")).as_um for wl in pump_wls_um
        ]
        ax2.plot(pump_wls_um, anti_wls, color=colors[name], linewidth=2, label=name)
    ...
    ax2.plot(
        ...
        linewidth=1,
        linestyle="--",
        label="Identity (pump = Anti-Stokes)",
    )
    ...
    plt.tight_layout()
    plt.savefig("examples/images/08_raman_pump_sweep.png", dpi=150, bbox_inches="tight")
    ...
    fig.suptitle(
        "Wavelength Shift Magnitude vs Pump Wavelength", fontsize=14, fontweight="bold"
    )
    ...
    ax.set_title("Equal Frequency Shift → Unequal Wavelength Shift", fontsize=13)
    ax.grid(True, alpha=0.3)
    ax.legend(fontsize=9, ncol=2)

    plt.tight_layout()
    plt.savefig(
        "examples/images/08_raman_shift_magnitude.png", dpi=150, bbox_inches="tight"
    )
    ...
    print("\n— Key Observations —")
    print(
        "• Frequency shift is CONSTANT for a given material (property of the molecule)"
    )
