...
def main():
    ...
    fwhm_fs = (
        envelope.fwhm.as_fs if hasattr(envelope.fwhm, "as_fs") else envelope.fwhm * 1e15
    )
    print(
    ...
    for name in materials:
        ...
        interaction = RamanPulseInteraction(pulse=wave, response=response, spec=spec)
        interactions[name] = interaction
        P_NL = interaction.nonlinear_polarization
        print(
            f"{name}: max|P_NL|={np.max(np.abs(P_NL)):.3e}, "
    ...
    plt.savefig(
        "examples/images/07_raman_pulse_interaction_silica.png",
        dpi=150,
        bbox_inches="tight",
    )
    ...
    fig.suptitle(
        "Nonlinear Polarization P_NL(t) — Material Comparison",
        fontsize=14,
        fontweight="bold",
    )
    ...
    for idx, name in enumerate(materials):
        ...
        ax.plot(
            t_ps, I_norm, color="#555555", linewidth=1.0, alpha=0.5, label="Input pulse"
        )
    ...
    plt.tight_layout()
    plt.savefig(
        "examples/images/07_raman_pulse_interaction_comparison.png",
        dpi=150,
        bbox_inches="tight",
    )
    ...
    fig.suptitle("Delayed Polarization Lag — Silica", fontsize=14, fontweight="bold")
    ...
    R_norm = (
        R_delayed / np.max(np.abs(R_delayed))
        if np.max(np.abs(R_delayed)) > 0
        else R_delayed
    )
    ...
    plt.tight_layout()
    plt.savefig(
        "examples/images/07_raman_pulse_interaction_lag.png",
        dpi=150,
        bbox_inches="tight",
    )
    ...
    fig.suptitle("Pulse Shape Effect on P_NL — Silica", fontsize=14, fontweight="bold")
    ...
    for idx, (shape, label) in enumerate(zip(shapes, shape_labels)):
        ...
        ax.plot(
            t_ps, I_norm, color="#00d4ff", linewidth=1.0, alpha=0.7, label="|E(t)|²"
        )
    ...
    plt.tight_layout()
    plt.savefig(
        "examples/images/07_raman_pulse_interaction_shapes.png",
        dpi=150,
        bbox_inches="tight",
    )
