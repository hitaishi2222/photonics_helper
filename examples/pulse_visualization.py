...
class PulseParameters(BaseModel):
    """Validated pulse parameters used by the dashboard and figure builder."""

    shape: Shape = Field(default=DEFAULT_SHAPE)
...
def parse_params(
    shape: str, t0: str, chirp: str, amplitude: str, theme: str = "light"
) -> tuple[Shape, float, float, float]:
    ...
    try:
        ...
        return cast(Shape, shape), t0_val, chirp_val, amp_val
...
async def pulse_update(
    ...
    try:
        shape, t0_val, chirp_val, amp_val = parse_params(
            shape, t0, chirp, amplitude, theme
        )
    except ValueError as e:
        return HTMLResponse(f"<div style='color:red; padding:20px;'>Error: {e}</div>")

    # Create a validated parameter object for _build_pulse_figure
    params = PulseParameters(
        shape=shape, t0=t0_val, chirp=chirp_val, amplitude=amp_val, theme=theme
    ...
    template_content = template_content.replace(
        "{{ params.amplitude }}", str(params.amplitude)
    )
