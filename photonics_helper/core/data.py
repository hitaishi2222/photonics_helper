"""SQLite materials-database backend (foundation layer).

Owns ``materials.db``: path resolution (bundled DB first, then a
user-home fallback), schema creation/migration, and the generic
read/write queries — Sellmeier rows, tabulated n/k data, provenance
records, phonon modes and the per-material spec table.

Historically this class (as ``RamanDatabase``) lived under the
``raman`` satellite; the bundled database is a *materials* database, so
the backend is a core primitive since Phase 1 of the backbone plan
(``core.data``). The historical access paths keep working:

    from photonics_helper.raman.db import RamanDatabase      # alias
    from photonics_helper.core.data import MaterialsDatabase  # canonical

The only satellite leaves are the lazily imported seeding tables and
the :class:`~photonics_helper.phonon.PhononMode` type adapter — both
imported at method level so the core import budget stays dependency-
clean.
"""
...
class MaterialsDatabase:
    """SQLite database for bundled optical-material data.
    ...
    def _seed_if_empty(self) -> None:
        ...
        if db_path != home_db.resolve():
            return
        from ..raman.reference import RAMAN_MATERIALS, THORLABS_SUBSTRATE_MATERIALS
    ...
    def get_phonon_modes(self, material: str) -> list[PhononMode]:
        ...
        """
        from ..phonon import PhononMode  # lazy: satellite leaf
